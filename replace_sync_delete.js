...
const deleteProductsRegex = /router\.delete\('\/products\/:id', auth, requireActiveSubscription, async \(req, res\) => \{[\s\S]*?\}\);\n/g;

const newDeleteProducts = `router.delete('/products/:id', auth, requireActiveSubscription, async (req, res) => {
  try {
    const { id } = req.params;
...
`;
content = content.replace(deleteProductsRegex, newDeleteProducts);


const deleteTxRegex = /router\.delete\('\/transactions\/:id', auth, requireActiveSubscription, async \(req, res\) => \{[\s\S]*?\}\);\n/g;
const newDeleteTx = `router.delete('/transactions/:id', auth, requireActiveSubscription, async (req, res) => {
  try {
    const { id } = req.params;
...
`;
content = content.replace(deleteTxRegex, newDeleteTx);
